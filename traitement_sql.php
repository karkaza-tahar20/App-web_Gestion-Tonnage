<?php 
	class DAO{
		...
		function inscription($username,$email,$password){
			$pdo=$this->connexion();
			$req="insert into utilisateurs(username,email,password) values('$username','$email','$password')";
			$rep=$pdo->query($req);
			if($rep->fetch()){
			$rep->closeCursor();
				return true;
		...
		function authentification1($login,$password){
			$pdo=$this->connexion();
			$req="select * from utilisateurs where username='".$login."' and password='".$password."'";
			$rep=$pdo->query($req);
			if($rep->fetch()){
			  $rep->closeCursor();
				return true;
