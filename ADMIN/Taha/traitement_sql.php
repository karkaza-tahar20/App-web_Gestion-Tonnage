<?php 
	class DAO{
		...
		function listeTrains(){
			...
			while($row=$rep->fetch()){
				$lstTrains[]=array("id_train"=>$row['id_train'],"Matricule"=>$row['Matricule'],"Adresse_Capteur"=>$row['Adresse_Capteur'],"Poids"=>$row['Poids'],"Direction"=>$row['Direction'],"Destination"=>$row['Destination'],"Date"=>$row['Date'],"Heur"=>$row['Heur']);
			}
			$rep->closeCursor();
			return $lstTrains;
		}
        ...
        function AfficherHist(){
			...
			while($row=$rep->fetch()){
				$lstTer[]=array("id"=>$row['id'],"Matricule"=>$row['Matricule'],"Date_S"=>$row['Date_S'],"Heur_S"=>$row['Heur_S'],"Poids_S"=>$row['Poids_S'],"Date_E"=>$row['Date_E'],"Heur_E"=>$row['Heur_E'],"Poids_E"=>$row['Poids_E'],"Destination"=>$row['Destination'],"Poids_G"=>$row['Poids_G']);
			}
			$rep->closeCursor();
			return $lstTer;
		}
		...
		function modTrain($id,$Matricule,$Adresse_Capteur,$Poids,$Direction,$Destination,$Date,$Heur){
			$pdo=$this->connexion();
			$lstEts=array();
			$rep=$pdo->query("update trains set Matricule='".$Matricule."', Adresse_Capteur='".$Adresse_Capteur."', Poids='".$Poids."', Direction='".$Direction."', Destination='".$Destination."', Date='".$Date."' ,Heur='".$Heur."' where id_train LIKE '".$id."' and Matricule LIKE '".$Matricule."'");

			//$rep->closeCursor();
		}
		function modTrainHist($id,$Matricule,$Poids,$Date,$Heur,$P){
			$pdo=$this->connexion();
			$lstEts=array();
			$rep=$pdo->query("update historique set Poids_E='".$Poids."', Date_E='".$Date."', Heur_E='".$Heur."', Poids_G=('".$Poids."' -'".$P."') where id LIKE '".$id."' and Matricule LIKE '".$Matricule."'");
			//$rep->closeCursor();
		}
		function modTrainHistSortie($id,$Matricule,$Poids,$Date,$Heur){
			$pdo=$this->connexion();
			$lstEts=array();
			$rep=$pdo->query("update historique set  Date_S='".$Date."', Heur_S='".$Heur."', Poids_S='".$Poids."' where id LIKE '".$id."' and Matricule LIKE '".$Matricule."' and Poids_G = '' ");
			//$rep->closeCursor();
		}
		...
		function rechercherTrainDatMat($Matricule,$Date){
			...
			while($row=$rep->fetch()){
				$lstTer[]=array("id_train"=>$row['id_train'],"Matricule"=>$row['Matricule'],"Adresse_Capteur"=>$row['Adresse_Capteur'],"Poids"=>$row['Poids'],"Direction"=>$row['Direction'],"Destination"=>$row['Destination'],"Date"=>$row['Date'],"Heur"=>$row['Heur']);
			}
			$rep->closeCursor();
			return $lstTer;
		}
		...
		function rechercherId($id){
			$pdo=$this->connexion();
			$lstTer=array();
			$rep=$pdo->query("select * from trains where id_train LIKE '%".$id."%' order by Date desc");
			while($row=$rep->fetch()){
				$lstTer[]=array("id_train"=>$row['id_train'],"Matricule"=>$row['Matricule'],"Adresse_Capteur"=>$row['Adresse_Capteur'],"Poids"=>$row['Poids'],"Direction"=>$row['Direction'],"Destination"=>$row['Destination'],"Date"=>$row['Date'],"Heur"=>$row['Heur']);
			}
			$rep->closeCursor();
		...
		function rechercherhist($Matricule){
			$pdo=$this->connexion();
			$lstTer=array();
			$rep=$pdo->query("select * from historique where Matricule  LIKE '".$Matricule."' AND Poids_G LIKE '' order by Date_S desc");
			while($row=$rep->fetch()){
				$lstTer[]=array("id"=>$row['id'],"Matricule"=>$row['Matricule'],"Date_S"=>$row['Date_S'],"Heur_S"=>$row['Heur_S'],"Poids_S"=>$row['Poids_S'],"Date_E"=>$row['Date_E'],"Heur_E"=>$row['Heur_E'],"Poids_E"=>$row['Poids_E'],"Destination"=>$row['Destination'],"Poids_G"=>$row['Poids_G']);
			}
			$rep->closeCursor();
		...
		function rechercherhistEntrer($Matricule,$Date){
			$pdo=$this->connexion();
			$lstTer=array();
			$rep=$pdo->query("select * from historique where Matricule  LIKE '".$Matricule."' AND Date_E LIKE '".$Date."'  order by Date_E desc");
			while($row=$rep->fetch()){
				$lstTer[]=array("id"=>$row['id'],"Matricule"=>$row['Matricule'],"Date_S"=>$row['Date_S'],"Heur_S"=>$row['Heur_S'],"Poids_S"=>$row['Poids_S'],"Date_E"=>$row['Date_E'],"Heur_E"=>$row['Heur_E'],"Poids_E"=>$row['Poids_E'],"Destination"=>$row['Destination'],"Poids_G"=>$row['Poids_G']);
				break;
			}
